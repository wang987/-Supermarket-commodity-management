...
{
	//按照销量排序
	public List<ProductImEx> daooperate(int nowPage,String string) throws SQLException {
		String sql="select p.productid productid, name,count,bid,price,profit,sum(salecount) salecount from product p,sales s,productsales ps where s.salesid=ps.salesid and p.productid=ps.productid  group by name ORDER BY salecount DESC limit "+(nowPage-1)*4+",4;";
		return super.getAllImExByKey(sql);
		
	}
	//按照利润排序
	public List<ProductImEx> daooperate(int nowPage) throws SQLException {
